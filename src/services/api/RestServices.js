...
class RestServices {
	...
	// post telecel postpaid
	async postHubtelTelecelPostpaid(Destination, Amount, ClientReference) {
		try {
			Log.info(
				`[RestServices.js][postHubtelTelecelPostpaid] \t recharging telecel postpaid`
			);
			const response = await axios.post(
				`https://cs.hubtel.com/commissionservices/${process.env.HUBTEL_PREPAID_DEPOSTI_ACCOUNT}/${process.env.HUBTEL_TELECEL_POSTPAID_BILLS_SERVICE_ID}`,
				{
					Destination: Destination,
					Amount: Amount,
					CallbackURL: `${process.env.HUBTEL_CALLBACK_BASE_URL}/api/v1/hubtel-airtime-callback`,
			...
			Log.info(
				`[RestServices.js][postHubtelTelecelPostpaid] error posting telecel postpaid: ${error.message}`
			);
			if (error.response) {
				Log.info(
					`[RestServices.js][postHubtelTelecelPostpaid] response status: ${error.response.status}`
				);
				Log.info(
					`[RestServices.js][postHubtelTelecelPostpaid] response data: ${JSON.stringify(
						error.response.data
					)}`
				);
			} else if (error.request) {
				Log.info(
					`[RestServices.js][postHubtelTelecelPostpaid] request: ${error.request}`
				);
			} else {
				Log.info(
					`[RestServices.js][postHubtelTelecelPostpaid] unknown error: ${error.message}`
				);
			}
			return {
				success: false,
